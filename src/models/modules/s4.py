...
try: # Try CUDA extension
    ...
    log.warning(
        "CUDA extension for cauchy multiplication not found. Go to extensions/cauchy/ and run `python setup.py install`. This should speed up end-to-end training by 10-50%"
    )
...
class S4_Layers(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(n_blocks):
            self.s4_blocks.append(S4(d_model=d_model,
                                     d_state = d_state,
                                     l_max = None,
                                     dropout=dropout,
                                     transposed=True,
    ...
    def forward(self, x):
        """
        Input x must be shape (B, L, d_model)
        """
        
        x = x.transpose(-1, -2)  # (B, L, d_model) -> (B, d_model, L)
        
        for layer, norm, dropout in zip(self.s4_blocks, self.norms, self.dropouts):
